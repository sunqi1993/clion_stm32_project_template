...
{ 					
    ...
    FSMC_NORSRAMInitStructure.FSMC_WaitSignalPolarity = FSMC_WaitSignalPolarity_Low;FSMC_NORSRAMInitStructure.FSMC_AsynchronousWait=FSMC_AsynchronousWait_Disable;
    ...
    //设置默认的显示方向
	LCD_Display_Dir(1);		//默认为竖屏0 横屏1
	LCD_LED=1;				//点亮背光1 关闭背光0
	LCD_Clear(WHITE);
}  
//清屏函数
//color:要清屏的填充色
...
//显示字符串
//x,y:起点坐标
//width,height:区域大小 绘图不能超过这个区域,但是这并不是一定是边界的大小
//size:字体大小
//*p:字符串起始地址		  
